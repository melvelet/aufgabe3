...
void DBMyQueryManager::selectJoinTupleIndexedNested(DBTable *table[2],
  ...
  selectTuple(table[inner], where[inner], innerlist);
  cout << "size: " << innerlist.size() << "\n";
  DBIndex * index = createTempIndex(innerlist, attrJoinPos[inner]);
  ...
  while (i != outerlist.end()) {
    ...
    //get index values from inner relation
    index->find(left.getAttrVal(attrJoinPos[inner]), tids);
    cout << "size: " << tids.size() << "\n";
    table[inner]->readTIDs(tids, templist);

    // DBListTID::iterator u = tids.begin();
    // while (u != l[1].end()) {
    //   DBTuple &right = (*u);
...
//Create new Index, add all entries from selectTuple(Inner))
DBIndex* DBMyQueryManager::createTempIndex(DBListTuple &tuplelist, uint attrJoinPos) {
  struct QualifiedName index_name = {"temp", "TEMP"};
  
  cout << "createTempIndex\n";
  sysCatMgr.createIndex(connectDB, index_name, "DBSeqIndex");
  cout << "here2\n";
  DBIndex * temp_index = sysCatMgr.openIndex(connectDB, index_name, READ);
  cout << "here3\n";
  DBListTuple::iterator u = tuplelist.begin();
    while (u != tuplelist.end()) {
      DBTuple &tid = (*u);
      temp_index->insert(tid.getAttrVal(attrJoinPos), tid.getTID());
    }
    cout << "here4\n";
  return temp_index;
}
